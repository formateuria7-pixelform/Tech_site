...
type Actions = {
  // Authentification
  register: (
    input: Omit<User, 'id' | 'role' | 'createdAt' | 'address'> & { address?: Address },
  ...
  // Devis
  createQuote: (
    input: Omit<Quote, 'id' | 'ref' | 'createdAt' | 'status' | 'userId' | 'messages'>,
  ) => Quote;
  ...
  // Rendez-vous
  createAppointment: (
    input: Omit<Appointment, 'id' | 'ref' | 'createdAt' | 'status' | 'userId'>,
  ) => Appointment;
    ...
    (set, get) => ({
      ...
      placeOrder: ({ paymentMethod, shippingAddress, billingAddress }) => {
        ...
        set((st) => ({
          ...
          logs: [
            { id: id('log'), date: now(), actor: 'boutique', action: `Commande ${order.ref} créée` },
            ...st.logs,
          ],
        }));
        return order;
      ...
      createQuote: (input) => {
        ...
        set((s) => ({
          ...
          logs: [
            { id: id('log'), date: now(), actor: input.contact.email, action: `Devis ${quote.ref} déposé` },
            ...s.logs,
          ],
        }));
        return quote;
          ...
          logs: [
            {
              id: id('log'),
              date: now(),
              actor: input.contact.email,
              action: `Rendez-vous ${appointment.ref} créé`,
            },
            ...s.logs,
          ],
        }));
      ...
      addProductReview: (slug, author, rating, comment) =>
        set((s) => ({
          catalog: s.catalog.map((p) =>
            p.slug === slug ? { ...p, reviews: [{ author, rating, date: now(), comment }, ...p.reviews] } : p,
          ),
        })),
      ...
      saveProduct: (product) =>
        set((s) => ({
          catalog: s.catalog.some((p) => p.slug === product.slug)
            ...
            : [product, ...s.catalog],
          logs: [
            { id: id('log'), date: now(), actor: 'admin', action: `Produit enregistré : ${product.name}` },
            ...s.logs,
          ],
        })),
      ...
      deleteProduct: (slug) =>
        set((s) => ({
          ...
          logs: [
            { id: id('log'), date: now(), actor: 'admin', action: `Produit supprimé : ${slug}` },
            ...s.logs,
          ],
        })),
      ...
      savePost: (post) =>
        set((s) => ({
          journal: s.journal.some((p) => p.slug === post.slug)
            ...
            : [post, ...s.journal],
          logs: [
            { id: id('log'), date: now(), actor: 'admin', action: `Article enregistré : ${post.title}` },
            ...s.logs,
          ],
        })),
      ...
      deletePost: (slug) =>
        set((s) => ({
          ...
          logs: [
            { id: id('log'), date: now(), actor: 'admin', action: `Article supprimé : ${slug}` },
            ...s.logs,
          ],
        })),
...
/* ---------- Sélecteurs dérivés ----------
 * RÈGLE : un sélecteur passé à useStore() ne doit JAMAIS créer d'objet ou de
 * tableau ({}, [], .map, .filter, .slice, .reduce vers un objet). Sinon React
 * détecte un nouveau « snapshot » à chaque rendu → boucle infinie
 * (« The result of getSnapshot should be cached » / « Maximum update depth »).
 * On sélectionne donc des valeurs atomiques, puis on dérive avec useMemo.
 */
...
export function useCartDetails(): CartDetails {
  // Sélecteurs atomiques : références stables issues du store.
  ...
  return useMemo(() => {
    ...
    const coupon = coupons.find((c) => c.code === appliedCoupon && c.active) ?? null;
    const discount = coupon ? (subtotal * coupon.percent) / 100 : 0;
    const shipping =
      lines.length === 0 || subtotal - discount >= FREE_SHIPPING_THRESHOLD ? 0 : SHIPPING_FLAT;
    const tax = (subtotal - discount) * TAX_RATE;
  ...
  }, [cart, catalog, coupons, appliedCoupon]);
}
