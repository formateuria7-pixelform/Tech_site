...
export function Account() {
  ...
  const quotes = useStore((s) => s.quotes.filter((q) => q.userId === user?.id));
  const appointments = useStore((s) => s.appointments.filter((a) => a.userId === user?.id));
  ...
  const favorites = catalog.filter((p) => favoriteSlugs.includes(p.slug));
  const unread = notifications.filter((n) => !n.read).length;

  return (
    <>
      <Seo title="Mon espace client" description="Suivi de vos commandes, devis, rendez-vous, favoris et documents." />
      <PageHeader
        ...
        <div className="mx-auto grid max-w-[1400px] gap-10 px-5 sm:px-6 lg:grid-cols-[250px_1fr] lg:gap-12 xl:px-10">
          ...
          <div>
            ...
            <div className="space-y-8">
                ...
                <li key={label as string}>
                    ...
                    className="block rounded-2xl border border-paper/10 bg-coal p-6 transition-colors hover:border-paper/25">
                        ...
                        <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.16em] text-fog">{label}</p>
                      </Link>
                    </li>
                )}
                </ul>
                ...
                {(orders.length > 0 || quotes.length > 0 || appointments.length > 0) &&
              <div className="rounded-3xl border border-paper/10 bg-coal p-7">
              ...
              <div className="rounded-3xl border border-paper/10 bg-coal p-10 text-center">
                    <h2 className="font-display text-xl font-semibold text-paper">Aucune activité pour le moment</h2>
            ...
            <div className="space-y-5">
              ...
              <article key={order.id} className="rounded-3xl border border-paper/10 bg-coal p-7">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          <h2 className="font-display text-lg font-semibold text-paper">Commande {order.ref}</h2>
            ...
            <div className="space-y-5">
              ...
              <article key={a.id} className="rounded-3xl border border-paper/10 bg-coal p-7">
                      ...
                      <dl className="mt-5 grid gap-4 border-t border-paper/10 pt-5 sm:grid-cols-2">
                        ...
                        {a.note &&
                  <div>
                            <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-fog">Précisions</dt>
            ...
            <div>
              ...
              <EmptyState title="Aucune notification" text="Les confirmations et mises à jour apparaîtront ici." /> :
...
}: {onSave: (patch: Record<string, unknown>) => void;onLogout: () => void;}) {
  const user = useCurrentUser()!;
